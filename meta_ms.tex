...
Our approach is to reanalyze the meta-analysis of \citet{Anderson:etal:2010}, and we do so for the following reasons:  First, there are now new and more effective techniques for addressing the evidentiary critiques.  These new techniques, including PET \citep[Precision-effect test,][]{Stanley:Doucouliagos:2014}, PEESE \citep[Precision-Effect Estimate with Standard Error,][]{Stanley:Doucouliagos:2014}, and $p$-curve \citep{Simonsohn:etal:2014,Simonsohn:etal:2014b}, provide for better control of publication bias and questionable research practices than those used in \citet{Anderson:etal:2010}.  Second, some are concerned about the application of the inclusion criteria \citep{Elson:Ferguson:2014}n.  Anderson et al. are admirably transparent in defining two working sets of studies---the full set of all studies, and an additional set of studies that meet {\em best practices} criteria.  As noted by Anderson et al., the best-practices set yields somewhat larger effects than the full set.  For example, the violent-video-game effect on aggressive affect is $r=.29$ for the best-practices experiments but only $r=.181$ for the full set.  
...
Because the data were analyzed using the software ``Comprehensive Meta-Analysis'' with the intent of testing for moderators, many studies were entered with separate rows for different subsamples within studies. However, we assume that entire studies are censored or re-analyzed per their statistical significance. Where multiple effects were entered for a single study (e.g., separate simple effects for men and women), we aggregated these to form a single study, summing the sample sizes and making a weighted average of the subsample effect sizes. This parallels the behavior of the software used in the original analysis. 

$p$-values were calculated via $t$-test, first dividing Fisher's Z scores by their standard errors to generate a $t$-value, then using that $t$-value to get a two-tailed $p$-value. As such, we do not report a $p$-value disclosure table as recommended by \citet{Simonsohn:etal:2014}, as the meta-analyzed $p$-values are a function of the data as entered by Anderson et al. and not a direct entry of $p$-values from manuscripts.
...
Finally, \citet{Graybill:etal:1985} was removed from analysis. As entered in the Anderson et al. dataset, the effect size was unusually large and significant, $r = 0.57, p = 1.6 \times 10^{-10}$. The cause of this enormous outcome was that the study's manipulation checks were entered as though they were primary study outcomes on aggressive cognitions. Participants were asked ``Tell me what happened in the video game you played,'' and ``What did you like about the video game you played?'' The results of the chi-square tests on these manipulation checks were then averaged together with two non-significant outcomes. While what is meant by ``aggressive cognition'' is unclear, we do not think it is supposed to be measured by such manipulation checks.
...
In the course of the experiment reported in \citet{Carnagey:Anderson:2005}, a nonexperimental assessment was also made of the effects of previous violent game exposure on aggressive outcomes as measured within the experiment. In the manuscript, nonsignificant effects of previous violent game exposure were reported for aggressive affect (study 1; $F(1, 66) = 0.78, r = -.11$), aggressive cognitions (study 2; $F(1, 57) = 0.02, r = .02$), and aggressive behavior (study 3; $F(1, 133) = 0.23, r = -.04$). These nonsignificant results were not entered for analysis. 
However, the significant correlations between previous violent game exposure and a questionnaire measure of trait physical aggression was entered. While we are confident that Anderson et al. mean well and see this latter correlation as a more-appropriate test than that between previous exposure and behavior within the experiment, we think both effect sizes should have been entered and averaged for analysis.
