...
Results of the $p$-curve, PET, and PEESE analyses are supplied in Table \ref{table:adjustment} alongside na{\"i}ve fixed-effects and random-effects estimates. We caution the reader that we do not yet know which adjusted estimator is likely to perform best; perhaps it is $p$-curve, which does not depend on a true or false null hypothesis like PET and PEESE do. However, a weighted combination of several estimators often outperforms any single estimator. Therefore, we suggest that the reader consider all five estimates and apply her own weights in deciding for herself what seems the most likely true effect in each subsample. 
...
One of us has published such an interaction, trait anger $\times$ violent game exposure \citep{Engelhardt:etal:2011a}, and later found that it did not replicate (Engelhardt, Hilgard, Clark, \& Mazurek, in prep). The diversity of reported moderators and the infrequency of their replication suggest possible weaknesses in the literature. \nocite{Engelhardt:etal:inprep}
...
In many ways, attempting to adjust for research bias in meta-analysis is a fool's errand. It is ultimately impossible to recover some ``true'' estimate by using the available, biased data to speculate about unavailable data that may or may not exist. For this reason, we feel that a collaborative registered replication report is the only way to obtain a credible and accurate estimate of the short-term effects of violent video games. In such an attempt, collaborators review and edit the proposed paradigm until all agree that the experiment provides a fair and effective test of the hypothesis. A sample of predetermined size is collected, and the results are published regardless of their statistical significance. This approach protects against biases caused by conditional stopping, flexible analysis, and publication pressures.
...
Modified-game paradigms may also help to alleviate the difficulties in interpreting tests of the effects of violent games on aggressive affect. We are concerned that how these experiments are analyzed and reported may make it impossible to report null results, possibly causing the pronounced bias we observed among these tests. Consider a hypothetical experiment comparing feelings of frustration caused by a violent and a non-violent video game. If the result is significant, this is interpreted as evidence that violent games cause aggressive feelings. However, if the result is not significant, this is not interpreted as evidence that violent games do not cause aggressive feelings, but rather evidence that the games are matched stimuli, differing only in violent content and not in other confounding dimensions.\footnote{See Anderson et al., 2010, page 156: ``Studies based on violent on violent and nonviolent video games that have been preselected to be equally arousing obviously are not appropriate tests of the short-term arousal- and affect-inducing effects of violent video games. Thus, they should be excluded from the analyses designed to test this specific hypothesis. The same is true when comparison games have been preselected to create equivalent affective states.''} Because the only difference between two well-modified games is the degree of violent content, the presence or absence of differences in aggressive affect can be tested without such ambiguity.
