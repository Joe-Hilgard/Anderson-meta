...
It is argued that trim-and-fill does a poor job of providing an adjusted effect size, adjusting too much when there is no bias and adjusting too little when there is bias \citep{Simonsohn:etal:2014b}. 
...
% TODO: Some softening with regard to high p-curve and p-uniform adjustments
The current set of estimators yield larger adjustments for bias than did Anderson et al.'s trim-and-fill estimators. In general,  

In experiments of aggressive affect, their analyses yielded no adjustment; ours yield an adjustment of -.14 or more.  The resulting effect size estimate is $r$ = .15 or lower. % Fix this

In experiments of aggressive behavior, their analyses recommended an adjustment of -.03 to $r$ = .18; ours recommend an adjustment ranging from -.06--.19, reducing $r$ to .15 or as little as .02.

In experiments of aggressive cognition, their analyses recommended an adjustment of -.02 to $r$ = .20; our analyses suggest that $r = .20$ could be accurate, but $r = .16$ is also possible.   
...
% Exceptions to the rule: PEESE higher for Agg-Aff best-only than all (but that includes crazy outlier)
%% Cross-sections of aggressive behavior rather higher
%% Physiological arousal considerably higher in PET and PEESE but not in p-curve/unif 
...
% Explain which outcome Ballard & Weist 1996 had
However, we note that the unusually large effect size estimate and unusually small $p$-value obtained in \citet{Ballard:Wiest:1996} may cause meta-regression to underestimate the effect and $p$-curve to overestimate the effect. Exclusion of this study in our sensitivity analyses brought the PET estimate ($r = .00$), $p$-curve estimate ($r = .19$), and $p$-uniform estimate ($r = .24$) into closer accord.
...
That said, effects on aggressive affect and aggressive cognition in experimental and cross-sectional research seem stronger and more robust, although $p$-curve, $p$-uniform, PET, and PEESE provide differing estimates of the strength of the adjusted effects.

Effects on aggressive cognition in experiments seem less contaminated by small-study effects, but may still represent some overestimation. Heterogeneity limits the strength of conclusions in this subset.

Effects on aggressive affect also appear overestimated, but our estimators disagree on the degree of bias. Meta-regression suggested minimal adjusted effects, whereas $p$-value-based adjustments suggested modest adjusted effects.

Effects in cross-sectional research seem robust to adjustment, but there is moderate heterogeneity. 
