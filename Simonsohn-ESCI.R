...
loss=function(t_obs,df_obs,d_est) { #Syntax t_obs: vector of t-values, df_obs of degrees of freedom, d_est: candidate d
...
{ loss.all=c() #Vector where results of fit for each candidate effect size are stored
  ...
  dhat=optimize(loss,c(dstart-.1,dstart+.1), df_obs=df_obs,t_obs=t_obs) #Now optimize in the neighborhood of that effect size
  #PLOT RESULTS
...
# 
# plotloss(t_obs=dat$t_approx[dat$top3=="y"], df_obs=dat$df[dat$top3=="y"], dmin=-.8, dmax=.8) # d = .365, r = .1795
# plotloss(t_obs=dat$t_approx[dat$top5VVG=="y"], df_obs=dat$df[dat$top5VVG=="y"], dmin=-.8, dmax=.8) # d = .315, r = .1556
# plotloss(t_obs=dat$t_approx[dat$top5VVG=="n"], df_obs=dat$df[dat$top5VVG=="n"], dmin=-.8, dmax=.8) # d = .163, r = .08
# 
